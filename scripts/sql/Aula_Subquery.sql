/*
 SCRIPT UTILIZADO NA AULA DE REFERNCIA DE SQL
 BANCO DE DADOS DESENVOLVIDO POR PROF. HEBERT
 https://github.com/hebertphp/sql192/blob/master/subquery_join.sql
...
-------------- CONSULTAS
SELECT * FROM funcionario; # Exibe todas as informações dos funcionários
SELECT MIN(salario) FROM funcionario; #Exibe o menor salário cadastrado
SELECT * FROM funcionario where salario = (SELECT MIN(salario) FROM funcionario); #Busca as informações dos funcionários cadastrados que possuem o menor salário cadastrado.
SELECT * FROM funcionario where salario > (SELECT AVG(salario) FROM funcionario); #Busca as informações dos funcionários cadastrados que possuem o salário acima da média salarial.

SELECT * from funcionario;
select nome from departamento;
select * from funcionario where departamento in (select nome from departamento); #Seleciona apenas os funcionarios cujo departamento esteja dentro dos parametros do IN, os parametros do IN são informados através do select na tabela departamento.
